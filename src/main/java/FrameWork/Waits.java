...
public class Waits {
    ...
    public WebElement esperarElementoFicarClicavel(By by){
        WebDriverWait espera = new WebDriverWait(driver, 100);
        return espera.until(ExpectedConditions.elementToBeClickable(by));
    }

    public WebElement esperarCarregarElemento(WebElement elemento){
        try {
            return new WebDriverWait(driver,30).until(ExpectedConditions.visibilityOf(elemento));
